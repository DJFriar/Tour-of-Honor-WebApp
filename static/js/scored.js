$(document).ready(() => {
  ...
  const scoredTable = $('#ScoredTable').DataTable({
    order: [[1, 'desc']],
    pageLength: 50,
    columnDefs: [
      {
  ...
  function setSubmissionFilter(category) {
    ...
    if (category === 'All') {
      scoredTable.column(5).search('').draw();
      scoredTable.rows({ order: 'current', search: 'applied' }).data();
    } else {
      scoredTable.column(5).search(category).draw();
      scoredTable.rows({ order: 'current', search: 'applied' }).data();
    }
  }
});
