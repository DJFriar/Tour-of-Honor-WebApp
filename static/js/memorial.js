$(document).ready(() => {
  ...
  const handlePrimaryPhoto = function (primaryInput) {
    if (primaryInput.files) {
      const reader = new FileReader();
      reader.onload = function (event) {
        $('.primaryImagePlaceholder').attr('src', event.target.result);
      };
  ...
  const handleOptionalPhoto = function (optionalInput) {
    if (optionalInput.files) {
      const reader = new FileReader();
      reader.onload = function (event) {
        $('.optionalImagePlaceholder').attr('src', event.target.result);
      };
